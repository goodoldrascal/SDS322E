...
```{r}
## Add your code here
glm_fit <- fit(wf1, data = train)
pred_class <- predict(glm_fit, test, type = "class")
pred_prob <- predict(glm_fit, test, type = "prob")
preds <- test %>%
  bind_cols(
    predict(glm_fit, test, type = "prob"),
    predict(glm_fit, test, type = "class")
  )
# evaluate AUC
roc_auc(preds, truth = injury, .pred_injury)
# evaluate accuracy
...
**Evaluation of our logistic regression model showed an accuracy of .597 and an AUC of .396.**
